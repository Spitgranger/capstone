...
\begin{itemize}
  ...
  \item Follows writing checklist (full checklist provided in a
    separate document)
    \begin{todolist}
    \item \LaTeX{} points
    \item Structure
  ...
  \item Overall qualities of documentation
    ...
    \item No statement is repeated at the same level of abstraction
      (for instance
        the scope should be more abstract than the assumptions, the
        goal statements
  ...
  \item Reference Material
    ...
    \item All non-SI units should be checked to see if they are really what is
      intended.  This also applies for unit modifiers, like m, k, c,
      etc.  That is,
  ...
  \item General System Description
    ...
    \item If the software will depend on other software, such as
      other libraries,
      ...
      are also specified.
    \item User characteristics are unambiguous (for instance, don't
        just say the user will know
        physics, say they will know Newtonian mechanics as typically
        covered in the
      first year of an engineering or science degree)
    \item User characteristics are specific
    \item System constraints have an appropriate rationale (a
        constraint without a
        reason for that constraint is likely making the SRS less
        abstract than it
      should be)
    \end{todolist}
  ...
  \item Solution Characteristics Specification
    ...
    \item If the ``Ref.\ By'' field is filled in, the entities
      (model, definition,
    ...
    \item All of the inputs for an IM are used in some way to define
      the output for the IM
